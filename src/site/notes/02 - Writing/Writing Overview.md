---
{"dg-publish":true,"permalink":"/02-writing/writing-overview/"}
---


# ✍️ Writing Overview - Vista General de Escritura

> La sección de Writing evalúa tu capacidad para escribir textos claros y bien estructurados en inglés.

> The Writing section tests your ability to write clear and well-structured texts in English.

---

## ⏱️ Exam Details / Detalles del Examen

- **Time / Tiempo:** 45 minutes total
- **Tasks / Tareas:** 2 tasks
- **Word count / Palabras:** ~100 words per task
- **Score / Puntuación:** 25% of total exam score

---

## 📋 Writing Parts Structure / Estructura de las Partes

### [[02 - Writing/Writing Part 1 - Email\|Part 1: Email]] (COMPULSORY / OBLIGATORIO)
**Task / Tarea:**
- Read an email from an English-speaking friend
- Write a reply email (~100 words)
- Answer all the points in the prompt

**What you must do / Qué debes hacer:**
- Respond to ALL notes/questions in the prompt
- Use appropriate email style
- Write about 100 words (90-110 is fine)

**Marks / Puntuación:**
- Content (5 marks) / Contenido
- Communicative Achievement (5 marks) / Logro comunicativo
- Organisation (5 marks) / Organización
- Language (5 marks) / Lenguaje

---

### [[02 - Writing/Writing Part 2 - Article\|Part 2: Article]] (OPTION 1 / OPCIÓN 1)
**Task / Tarea:**
- Write an article for a magazine, website, or newsletter
- Answer the question or respond to the topic given
- Write about 100 words

**Characteristics / Características:**
- Engaging title / Título atractivo
- Interesting opening / Apertura interesante
- Clear paragraphs / Párrafos claros
- Personal opinion / Opinión personal

---

### [[02 - Writing/Writing Part 2 - Story\|Part 2: Story]] (OPTION 2 / OPCIÓN 2)
**Task / Tarea:**
- Write a story
- Your story must begin with the sentence given
- Write about 100 words

**Characteristics / Características:**
- Clear beginning, middle, end / Inicio, desarrollo, final claros
- Interesting plot / Trama interesante
- Past tenses / Tiempos pasados
- Descriptive language / Lenguaje descriptivo

---

## 🎯 Assessment Criteria / Criterios de Evaluación

All tasks are marked on 4 criteria / Todas las tareas se evalúan con 4 criterios:

### 1. **Content (0-5 marks) / Contenido**
- Have you addressed ALL the points?
- Is the content relevant and appropriate?

**Tip / Consejo:** 🇪🇸 Responde TODOS los puntos en el prompt. Si falta uno, pierdes puntos.

---

### 2. **Communicative Achievement (0-5 marks) / Logro Comunicativo**
- Is the style appropriate for the task?
- Would it have the intended effect on the reader?

**For emails:** Friendly, natural tone
**For articles:** Engaging, interesting style
**For stories:** Entertaining, engaging narrative

---

### 3. **Organisation (0-5 marks) / Organización**
- Is the text well organized?
- Are ideas connected logically?
- Are paragraphs clear?
- Are linking words used correctly?

**Key elements / Elementos clave:**
- Clear paragraphs
- Logical flow
- Good use of connectors

---

### 4. **Language (0-5 marks) / Lenguaje**
- Range of vocabulary and grammar
- Accuracy (spelling, grammar, punctuation)

**Note / Nota:** You need BOTH variety and accuracy!

---

## 💡 General Writing Strategies / Estrategias Generales

### Before Writing / Antes de Escribir:

1. **Read the prompt CAREFULLY** / Lee el prompt CON CUIDADO
   - Underline key points to address
   - Note the task type and target reader

2. **Plan your answer** (2-3 minutes)
   - Main points for each paragraph
   - Ideas and examples
   - Opening and closing

3. **Choose appropriate language**
   - Formal vs informal
   - Vocabulary for the topic

---

### While Writing / Mientras Escribes:

1. **Follow your plan**
   - Clear paragraphs
   - One main idea per paragraph

2. **Use variety**
   - Different sentence structures
   - Range of vocabulary
   - Various connectors

3. **Check word count**
   - About 100 words
   - 90-110 is acceptable
   - Don't go too far under or over

---

### After Writing / Después de Escribir:

**ALWAYS leave 5 minutes to check! / ¡SIEMPRE deja 5 minutos para revisar!**

1. **Content:** Did you answer all points?
2. **Organisation:** Clear paragraphs? Good flow?
3. **Language:** Check spelling, grammar, punctuation
4. **Length:** Count words (approximately)

---

## ⏰ Time Management / Gestión del Tiempo

Suggested timing / Tiempo sugerido:

| Activity | Part 1 (Email) | Part 2 (Article/Story) |
|----------|----------------|------------------------|
| Read & Plan | 3-4 min | 3-4 min |
| Write | 12-14 min | 12-14 min |
| Check | 3 min | 3 min |
| **TOTAL** | **~20 min** | **~20 min** |
| Reserve | 5 min for final check |

**Total: 45 minutes**

---

## 📝 Word Count Guidance / Guía de Cantidad de Palabras

**Target: ~100 words per task**

- ✅ **90-110 words:** Perfect!
- ⚠️ **80-90 or 110-120:** Acceptable but be careful
- ❌ **Under 80:** Too short - probably missing content
- ❌ **Over 120:** Too long - wastes time, more chance of errors

**Tip / Consejo:**
- 🇪🇸 No cuentes cada palabra en el examen. Con práctica, sabrás aproximadamente cuánto es 100 palabras.
- Don't count every word in the exam. With practice, you'll know roughly what 100 words looks like.

---

## ✅ Dos and Don'ts / Hacer y No Hacer

### ✅ DO / HACER:

- ✅ Answer ALL points in the prompt
- ✅ Use clear paragraphs (3-4 paragraphs ideal)
- ✅ Use a variety of grammar and vocabulary
- ✅ Check your work (spelling, grammar, punctuation)
- ✅ Write clearly and legibly
- ✅ Stay on topic
- ✅ Use appropriate style for the task

### ❌ DON'T / NO HACER:

- ❌ Copy long phrases from the prompt
- ❌ Write too much or too little
- ❌ Use very informal language (unless appropriate)
- ❌ Make paragraphs too long or too short
- ❌ Use words you're not sure about
- ❌ Leave out any prompt points
- ❌ Forget to check your work

---

## 🔑 Key Language Areas / Áreas Clave del Lenguaje

### 1. **Connectors / Conectores**
Learn to use these naturally:
- Addition: and, also, moreover, furthermore
- Contrast: but, however, although, while
- Result: so, therefore, as a result
- Examples: for example, such as, like
- Sequence: first, then, after that, finally

### 2. **Paragraph Structure / Estructura de Párrafos**
- **Opening:** Introduce topic / Introduce el tema
- **Body:** Develop ideas / Desarrolla ideas
- **Closing:** Conclude or summarize / Concluye o resume

### 3. **Variety in Sentences / Variedad en Oraciones**
Mix different types:
- Simple: "I love reading."
- Compound: "I love reading and I visit the library often."
- Complex: "I love reading because it helps me relax."

---

## 📚 Common Topics / Temas Comunes

Be prepared to write about:
- 🎓 **Education and learning** / Educación y aprendizaje
- 🏠 **Daily life and routines** / Vida diaria y rutinas
- 🎮 **Hobbies and free time** / Pasatiempos y tiempo libre
- ✈️ **Travel and holidays** / Viajes y vacaciones
- 👨‍👩‍👧‍👦 **Family and friends** / Familia y amigos
- 💻 **Technology** / Tecnología
- 🌍 **Environment** / Medio ambiente
- 🏋️ **Sports and health** / Deportes y salud
- 🎬 **Entertainment** / Entretenimiento
- 💼 **Work and future plans** / Trabajo y planes futuros

---

## 📖 Self-Assessment Questions / Preguntas de Autoevaluación

After practicing, ask yourself / Después de practicar, pregúntate:

**Content / Contenido:**
- ✅ Did I answer all the points?
- ✅ Is my content relevant?

**Organisation / Organización:**
- ✅ Are my paragraphs clear?
- ✅ Is there a logical flow?
- ✅ Did I use connectors well?

**Language / Lenguaje:**
- ✅ Did I use a variety of grammar?
- ✅ Did I use a range of vocabulary?
- ✅ Is my spelling and grammar accurate?

**Style / Estilo:**
- ✅ Is my style appropriate for the task?
- ✅ Would a reader find it engaging?

---

## 🎯 How to Practice / Cómo Practicar

1. **Write regularly** / Escribe regularmente
   - Practice both email and article/story
   - Time yourself
   - Follow exam conditions

2. **Get feedback** / Obtén retroalimentación
   - Ask a teacher to check your writing
   - Use online writing checkers
   - Compare with model answers

3. **Learn from models** / Aprende de modelos
   - Read example answers
   - Note good phrases and structures
   - Adapt them to your own writing

4. **Build a phrase bank** / Construye un banco de frases
   - Collect useful phrases for each task type
   - Practice using them
   - Make them natural

5. **Focus on weaknesses** / Enfócate en debilidades
   - If grammar is weak → study and practice
   - If vocabulary is limited → learn topic vocabulary
   - If organisation is poor → practice paragraph structure

---

## 🔗 Related Notes / Notas Relacionadas

### Writing Parts / Partes de Escritura:
- [[02 - Writing/Writing Part 1 - Email\|Writing Part 1 - Email]]
- [[02 - Writing/Writing Part 2 - Article\|Writing Part 2 - Article]]
- [[02 - Writing/Writing Part 2 - Story\|Writing Part 2 - Story]]

### Support Materials / Materiales de Apoyo:
- [[Useful Phrases for Emails\|Useful Phrases for Emails]]
- [[Useful Phrases for Articles\|Useful Phrases for Articles]]
- [[Useful Phrases for Stories\|Useful Phrases for Stories]]
- [[05 - Grammar/Linking Words and Connectors\|Linking Words and Connectors]]
- [[Common Writing Mistakes\|Common Writing Mistakes]]

### Practice / Práctica:
- [[Writing Templates\|Writing Templates]] - In [[08 - Resources\|08 - Resources]]
- [[07 - Practice Tests\|Practice Tests]]

### Other Skills / Otras Habilidades:
- [[01 - Reading\|Reading Section]]
- [[03 - Listening\|Listening Section]]
- [[04 - Speaking\|Speaking Section]]

---

**Next step / Siguiente paso:** Study each writing task in detail, starting with [[02 - Writing/Writing Part 1 - Email\|Writing Part 1 - Email]].

🏠 [[00 - Start Here/🏠 Home - Inicio\|Back to Home / Volver al Inicio]]
